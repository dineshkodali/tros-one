...
const ExportModal: React.FC<ExportModalProps> = ({ isOpen, onClose, data, collectionName, columns }) => {
  ...
  const handleExport = () => {
    ...
    const finalData = sortedData.map(item => {
      ...
      selectedColumns.forEach(key => {
        // Handle nested or virtual fields safely
        if (key === '_assignments') {
             filteredItem['Assigned'] = item[key]?.map((x:any) => x.name).join(', ') || '';
        } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-zoom-in">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <h3 className="font-bold text-lg text-gray-900">Export {collectionName}</h3>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
               <label className="text-sm font-semibold text-gray-700 block mb-1">Sort By</label>
               <select 
                 value={sortBy} 
        ...
        <div className="p-4 border-t border-gray-100 flex justify-end gap-3 bg-gray-50">
          ...
          <button onClick={handleExport} className="px-4 py-2 bg-mono-primary text-white text-sm font-bold rounded-lg hover:bg-mono-secondary flex items-center gap-2">
