  ...
  return (
    <div className="max-w-6xl mx-auto h-full flex flex-col bg-white md:rounded-xl md:border md:border-mono-light md:shadow-sm overflow-hidden animate-fade-in">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar bg-white">
        {/* Shared Tabs */}
        {activeTab === "profile" && (
          <ProfileTab currentUser={currentUser} userRole={userRole} />
        )}
        {activeTab === "notifications" && <NotificationsTab />}

        {/* Vendor Security */}
        {activeTab === "security" && userRole === UserRole.VENDOR && (
          <VendorSecurityTab />
        )}
      ...
      if (userRole === UserRole.VENDOR) {
        const fetchShops = async () => {
          ...
          if (!vendorSnap.empty) {
            ...
            setVendorShops(
              shopsSnap.docs.map((s) => ({ id: s.id, ...(s.data() as any) }))
            );
          }
          setLoading(false);
        };
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                ...
                className="px-4 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-sm font-bold transition-colors"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:border-[#064e3b] focus:ring-1 focus:ring-[#064e3b] disabled:opacity-70 disabled:cursor-not-allowed transition-all"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wide">
              ...
              <div className="space-y-1.5 md:col-span-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 p-2.5 bg-gray-100 border border-gray-200 rounded-lg text-sm text-gray-500 cursor-not-allowed"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-900 rounded-xl p-6 shadow-lg relative overflow-hidden text-white border border-gray-700">
              ...
              <div className="flex justify-between items-start mb-8 relative z-10">
                <div>
                  ...
                  <p className="text-gray-400 text-xs mt-1">
                    For Order Remittances
                  </p>
                </div>
                <div className="text-right">
                  <span className="block font-bold text-[#064e3b]">
                    TROS BANK
                  </span>
              ...
              <div className="space-y-4 relative z-10">
                ...
                <div className="flex justify-between items-end border-b border-gray-700 pb-2">
                  ...
                  <span className="font-mono font-bold text-xl tracking-widest text-[#064e3b]">
                    8829 3910 2039
                  </span>
                </div>
            ...
            {userRole === UserRole.VENDOR && (
              <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm h-full">
                <h4 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
                  <Store size={18} className="text-[#064e3b]" /> My Shops
                </h4>
                {vendorShops.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {vendorShops.map((shop) => (
                      <div
                        key={shop.id}
                        className="flex justify-between items-center p-3 bg-gray-50 border border-gray-100 rounded-lg hover:border-emerald-200 transition-colors group cursor-pointer"
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs text-gray-500 flex items-center gap-1">
                            <Smartphone size={10} /> {shop.location}
                          </p>
                        </div>
                        <ArrowRight
                          size={14}
                          className="text-gray-300 group-hover:text-[#064e3b] transition-colors"
                        />
                      </div>
                    ))}
                  </div>
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm h-full">
              <div className="flex justify-between items-center mb-4">
                <h4 className="font-bold text-gray-900 flex items-center gap-2">
                  <File size={18} className="text-blue-500" /> Documents
                </h4>
                ...
                <tr key={u.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-4 text-right whitespace-nowrap">
                    <button
                      ...
                      onClick={() =>
                        triggerSonar("Delete restricted in demo", "info")
                      }
                    >
                      <Trash2 size={16} />
                    </button>
...
const AddUserModal = ({
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.email || !form.role || !form.joiningDate) {
      triggerSonar("All fields are required", "error");
      return;
    }
    ...
    try {
      ...
      await setDoc(doc(collection(db, "users")), {
        ...
        email: form.email,
        role: form.role,
        createdAt: new Date(form.joiningDate).toISOString(),
        status: "active",
      });

      triggerSonar("User created successfully");
      onUserCreated();
      onClose();
      ...
      console.error(err);
      triggerSonar("Failed to create user", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] bg-black/60 backdrop-blur-sm animate-fade-in">
      {/* Centering wrapper – SAME AS EXPORT MODAL */}
        ...
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-zoom-in">
          ...
          <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            <input
              ...
              className="w-full p-3 rounded-xl border bg-gray-50 focus:ring-1 focus:ring-[#064e3b] outline-none"
            />
          ...
          <div className="p-4 border-t border-gray-100 flex justify-end gap-3 bg-gray-50">
            ...
            <button
              ...
              disabled={loading}
              className="px-5 py-2 bg-[#064e3b] text-white text-sm font-bold rounded-lg hover:brightness-110 disabled:opacity-60"
            >
...
//     triggerSonar("User deleted successfully");
//   } catch (err) {
//     console.error(err);
//     triggerSonar("Failed to delete user");
//   }
// };
...
const RolesTab = () => {
  ...
  const togglePermission = (role: string, perm: string) => {
    ...
    if (current.includes(perm)) {
      setPermissions({
        ...permissions,
        [role]: current.filter((p: string) => p !== perm),
      });
    } else {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {["Administrator", "Vendor"].map((role) => (
        <div
          key={role}
          className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm hover:border-emerald-200 transition-colors"
        >
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        ...
        <button className="w-full mt-6 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-bold hover:bg-gray-200">
          Trigger Backup
        </button>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
          <Mail size={18} /> SMTP Configuration
        </h3>
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="notifications@tros.one"
          ...
          <button
            ...
            className="w-full mt-2 py-2 bg-[#064e3b] text-white rounded-lg text-sm font-bold hover:bg-[#065f46] flex items-center justify-center gap-2"
          >
    ...
    <div className="bg-gray-900 text-green-400 p-4 rounded-xl font-mono text-xs h-96 overflow-y-auto shadow-inner custom-scrollbar">
      ...
      {logs.map((log, i) => (
        <div key={i} className="mb-1 border-b border-gray-800 pb-1">
          ...
          <span className="text-yellow-400">{log.type}</span>: {log.description}{" "}
          <span className="opacity-50">by {log.user}</span>
        </div>
      ))}
    </div>
  );
...
const VendorSecurityTab = () => (
  <div className="text-center text-gray-500 p-10">
    Security settings restricted. Contact Admin to reset password.
  </div>
);
