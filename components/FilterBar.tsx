...
import { Search, X, ChevronDown, ArrowUpDown, ArrowUp, ArrowDown, Filter } from 'lucide-react';
  ...
  return (
    <div className="flex justify-center w-full mb-6">
      <div className="w-full max-w-4xl flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3 overflow-x-auto no-scrollbar py-1 px-1 -mx-4 md:mx-0 px-4 md:px-0">
          ...
          <div className="relative group shrink-0">
             <div className="flex items-center bg-white border border-emerald-100 rounded-lg h-10 pl-3 pr-2 shadow-sm cursor-pointer hover:border-[#064e3b] transition-colors">
                <span className="text-xs font-bold text-gray-500 uppercase tracking-wide mr-2">Sort</span>
                <select
                  value={sortConfig?.key || ''}
                  onChange={(e) => onSortChange(e.target.value, sortConfig?.direction || 'asc')}
                ...
                <ChevronDown size={14} className="absolute right-9 text-[#064e3b] pointer-events-none"/>
                
                <div className="w-px h-5 bg-gray-200 mx-2"></div>
                
                <button 
                  onClick={() => onSortChange(sortConfig?.key || sortOptions[0].key, sortConfig?.direction === 'asc' ? 'desc' : 'asc')}
          ...
          {filterOptions.map((option) => (
            <div key={option.key} className="relative shrink-0">
              {option.type === 'select' ? (
                <div className={`relative flex items-center h-10 px-3 rounded-lg border transition-all cursor-pointer ${
                    filters[option.key] 
                  ...
                  <select
                    ...
                    className={`appearance-none bg-transparent text-sm font-medium outline-none pr-5 cursor-pointer w-full ${filters[option.key] ? 'text-white font-bold' : ''}`}
                  >
                    <option value="" className="text-gray-500">{option.label}</option>
                    {option.options?.map((opt) => (
                      <option key={opt} value={opt} className="text-gray-900">{opt}</option>
                    ))}
                  </select>
                  <ChevronDown className={`absolute right-2 w-3 h-3 pointer-events-none ${filters[option.key] ? 'text-white' : 'text-gray-400'}`} />
                </div>
              ) : (
                <div className={`relative flex items-center h-10 bg-white border rounded-lg px-3 shadow-sm min-w-[140px] transition-colors ${filters[option.key] ? 'border-[#064e3b] ring-1 ring-[#064e3b]/20' : 'border-emerald-100'}`}>
                   <input
                      type="text"
                      value={filters[option.key] || ''}
                      onChange={(e) => onFilterChange(option.key, e.target.value)}
                   ...
                   />
                   {filters[option.key] && (
                      <button onClick={() => onFilterChange(option.key, '')} className="ml-1 text-gray-400 hover:text-red-500">
                         <X size={14}/>
                      </button>
                   )}
                </div>
